...
SYSTEM_PROMPT = (
    ...
    # note in agent_delivery.delivery_note. The last step of the chain is being rebuilt, so
    # for now the honest end state of a shipped app is "image built", not "deployed".
    "THE LAST STEP IS CURRENTLY OFFLINE: images get built, and nothing deploys them yet. "
    "That is temporary and it is not your problem to fix, but it does change what you may "
    "claim. There is no cluster URL for anything you ship right now — do not invent one, do "
    "not offer one as pending, and do not go looking for one.\n"
    "Leave your local server running anyway once you are done with it — it costs nothing and "
    "lets a human look at what you built. Just be honest about which URL is which: the local "
    "one is a preview that dies with the container, and a cluster address does not exist "
    "yet.\n"
    "\n"
