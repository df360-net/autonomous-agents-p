...
SYSTEM_PROMPT = (
    ...
    "the port and the address, deploys it when the image is built, and emails the live URL "
    "into the same conversation you are answering. None of that is yours to do or to predict: "
    "do not compute a cluster URL, do not repeat the one ship_app prints, and do not promise "
    "an address. Say it is built and registered and that the address will follow.\n"
    ...
    "one is a preview that dies with the container, and the cluster address is not yours to "
    "give.\n"
    "\n"
