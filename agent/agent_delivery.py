...
def delivery_note(assets_text, has_token):
    ...
    return (
        ...
        "\n"
        "SO REPORT THE ADDRESS `ship_app` PRINTS, EXACTLY AS PRINTED — and do not compute one "
        "of your own. Those are two different instructions and only the second is a rule about "
        "invention. The address IS the deliverable: a reply saying the app is built and that a "
        "URL will follow is a receipt, not an app, and it asks the person who requested the "
        "work to wait for a second email before they can look at anything. Say it may take a "
        "minute or two to start serving while the image deploys, and leave it there — the "
        "control plane sends its own confirmation once the pod is actually up.\n"
        "\n"
        "YOUR FRONT-END CODE MUST USE RELATIVE URLS. The app is served under a PATH PREFIX, "
        "https://apps.<fleet>/<app-name>/, not at the root of a host. `fetch('/api/vote')` "
        "works against your local preview and asks the SITE ROOT in the cluster: a 404, on a "
        "page that renders perfectly. Nothing reports an error — pod running, route serving, "
        "certificate valid, tests green — and the first person to click a button finds it. "
        "Write `fetch('api/vote')` and `src=\"logo.png\"`, no leading slash, and they resolve "
        "under any prefix including the root you develop against. Server-rendered links read "
        "the prefix from the BASE_PATH environment variable, which is set for you.\n"
        "\n"
        "NEVER PUT AN ADDRESS THE READER CANNOT OPEN INTO AN EMAIL. Your local preview is "
        "worth mentioning — it lets a human look at the work while it is reviewed — but "
        "`localhost` names the READER's machine, not yours, so that URL is an invitation to a "
        "connection error. Describe it as a local preview inside this container and give no "
        "URL for it.\n"
        "\n"
